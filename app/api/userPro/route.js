...
export async function POST(req) {
  try {
    ...
    // Vérification si le numéro de siret existe déjà
    ...
    const newCompany = await db.company.create({
      data: {
        ...
        adresse,
        typeSociete,
        // Prisma gère les champs `createdAt` et `updatedAt` automatiquement
      },
    });
    ...
    // Association de l'image avec l'utilisateur si elle existe
    if (imageUrl) {
    ...
    const account = await stripe.accounts.create({
      ...
      individual: {
        email: newComptePro.email,
        first_name: newComptePro.prenom,
        last_name: newComptePro.last_name,
      },
      metadata: {
        userId: newComptePro.id,
        userRole: newComptePro.role,
      },
      default_currency: "eur",
    });

    await db.user.update({
      where: { id: newComptePro.id },
      data: {
